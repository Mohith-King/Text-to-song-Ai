...
const MainPage4 = () => {
  ...
  return (
    <div className="flex flex-col items-center  justify-center no-scrollbar  mt-10 p-6 h-screen">
      ...
      <div className="flex gap-10 mt-10">
        {steps.map((e) => {
          return (
            <div key={e.id} className="flex flex-col p-4 h-1/2 rounded-xl w-1/3  bg-zinc-900">
              <div className="bg-white w-10 mb-4 h-10 rounded-[50%] flex justify-center items-center text-xl font-semibold "> {e.id} </div>
              <h1 className="text-white mt-2 mb-2 text-xl font-bold"> {e.title} </h1>
              <p className="text-md font-semibold text-zinc-300"> {e.desc} </p>
            </div>
          );
        })}
      </div>
